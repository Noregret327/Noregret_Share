...
## 十、An FPGA-based RNN-T inference accelerator with PIM-HBM

### 《基于fpga的PIM-HBM RNN-T推理加速器》

我们使用 FPGA 和 PIM-HBM 实现了世界优先 RNN-T 推理加速器，该加速器可以将内存的内部带宽相乘。加速器将RNN-T中LSTM层的矩阵向量乘法(GEMV)操作卸载到PIM-HBM中，PIM-HBM通过利用HBM内部带宽显著降低GEMV的执行时间。为了确保内存命令以预定义的顺序发出，这是利用PIM-HBM的最重要约束之一，我们利用FPGA的灵活性和可重构性实现了片上内存控制器的直接内存访问(DMA)模块和更改配置。此外，我们设计了其他用于加速的硬件模块，例如非线性函数（即 sigmoid 和双曲正切）、元素操作和 ReLU 模块，以在 FPGA 上运行这些计算绑定 RNN-T 操作。为此，我们准备了 FP16 量化权重和 MLPerf 输入数据集，并修改了基于 PCIe 设备驱动程序和基于 C++ 的控制代码。在我们的评估中，我们的PIM-HBM加速器平均将RNN-T的执行时间减少了2.5 ×，LUT大小减少了1.09%，比基线提高了2.6 ×的能量效率。

![image-20230828113317256](https://raw.githubusercontent.com/Noregret327/picture/master/202308281133295.png)



![image-20230828113336685](https://raw.githubusercontent.com/Noregret327/picture/master/202308281133757.png)

在本文中，我们使用带有PIM-HBM的FPGA实现了世界优先的RNN-T推理加速器，该加速器可以利用内部内存带宽。我们通过使用现有内存控制器的有序 DMA 模块和配置来成功启用 PIM 函数。我们的PIM-HBM加速器将RNN-T的执行时间平均减少了2.5倍，LUT大小减少了1.09%，与具有正常HBM立方体的基线加速器相比，能效提高了2.6倍。尽管我们的加速器以 300 MHz 的核心时钟频率运行，但当为所有系统提供相同的内存带宽时，它显示出比最先进的 GPU 系统更好的性能 1.98 倍。这项工作显示了系统级的性能增益以及具有 FPGA 灵活性和可编程性的 PIM 感知硬件系统设计，并为更广泛地采用和进一步发展 PIM 和相关生态系统打开了大门。





## 十一、FILM-QNN: Efficient FPGA Acceleration of Deep Neural Networks with Intra-Layer, Mixed-Precision Quantization

### 《FILM-QNN：具有层内混合精度量化的深度神经网络的高效 FPGA 加速》

我们提出了一个通用框架，称为<font color=red size=4>FILM-QNN</font>，用于量化和加速不同嵌入式FPGA设备上的多个DNN模型。首先，我们<font color=blue size=4>提出了一种新的层内混合精度量化算法，该算法将不同的精度分配给每一层的滤波器。候选精度水平和分配粒度由我们的实证研究确定，具有保持精度和提高硬件并行性的能力</font>。其次，我们对 FPGA 加速器架构应用了多种优化技术，以支持量化计算，包括 DSP 打包、权重重新排序和数据打包，以提高可用资源的整体吞吐量。此外，开发了一个综合资源模型来平衡FPGA计算资源(LUTs和DSPs)的分配，以及数据传输和片上存储资源(BRAMs)，以加快每层混合精度的计算。最后，为了提高FILM-QNN的可移植性，我们在Xilinx PYNQ-Z2和ZCU102 FPGA板上使用Vivado高级合成(HLS)实现它。<font color=green>我们在 ResNet-18、ResNet-50 和 MobileNet-V2 的实验结果表明，具有层内混合精度（95% 的 4 位权重和 5% 的 8 位权重以及所有 5 位激活）的实现可以达到与 8 位（和 32 位）版本相当的准确度（三个模型的 70.47%、77.25% 和 65.67%）版本和可比较的吞吐量（ZCU102 上的 214.8 FPS、109.1 FPS 和 537.9 FPS）作为 4 位设计。</font>

随着 DNN 在各个领域都取得了巨大的成功，它们越来越多地部署在嵌入式设备（例如嵌入式 FPGA 和 ASIC）上的推理。越来越多的具有复杂 DNN 架构的参数和计算使得模型压缩成为这些边缘设备上 DNN 加速的基本过程，其中计算和存储资源稀缺。作为关键模型压缩技术的一种类型，

模型量化在算法级别[2 - 4，6 - 10，12，17，18，22 - 26，29，34，36，39，40，44，45，49，51，54 - 56]

硬件级别[5，14 - 16，20，28，30 - 32，35，42，43，48，52]

都得到了广泛的研究，以提高可用资源的推理吞吐量。

事实上，业界已经广泛采用8位量化，可以实现与基于32位浮点的DNN模型的几乎相同的推理精度。在这里，我们主要讨论使用低于 8 位精度的量化技术。
